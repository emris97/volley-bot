...
describe('group onboarding presenter', () => {
  it.each([
    ...
    [{ tz: 'Europe/Astrakhan' }, 'Шаг 2 из 7', ['Участники группы выше гостей', 'В порядке записи']],
    [{ tz: 'Europe/Astrakhan', mp: true }, 'Шаг 3 из 7', ['За 24 часа', 'За 12 часов', 'За 6 часов']],
    [{ tz: 'Europe/Astrakhan', mp: true, tp: 1440 }, 'Шаг 4 из 7', ['30 минут', '1 час', '2 часа']],
    [{ tz: 'Europe/Astrakhan', mp: true, tp: 1440, tr: 60 }, 'Шаг 5 из 7', ['За 30 минут', 'За 1 час', 'За 2 часа']],
    [{ tz: 'Europe/Astrakhan', mp: true, tp: 1440, tr: 60, rm: 120 }, 'Шаг 6 из 7', ['Точно до копеек', 'Вверх до 1 ₽', 'Вверх до 10 ₽', 'Вверх до 50 ₽']],
    [{ tz: 'Europe/Astrakhan', mp: true, tp: 1440, tr: 60, rm: 120, ro: 'UP_10' }, 'Шаг 7 из 7', ['Да', 'Нет']],
  ] as Array<[WizardProgress, string, string[]]>)('renders the authoritative step for progress %#', (progress, heading, labels) => {
  ...
  it('renders safe Russian explanations for every expected start error', () => {
    ...
    expect(renderStartError('FOREIGN_LINK').text).toContain('другого администратора');
    expect(renderStartError('ADMIN_REQUIRED').text).toContain('права администратора');
  });
});
